...
def prepare_data():
    ...
    with open(f'{DATA_PATH}/sentence_latent-train-128dim-Nov2020.pkl', 'rb') as f:
    ...
    data = []
    not_found = 0
    for _id, o in enumerate(text_mu):
        if _id+1 > BATCH_SIZE*(len(text_mu)//BATCH_SIZE):
            continue
        spec_id = o['audioClip']
        ...
        try:
            data.append({'spec_mu': modified_img_mu[spec_id],
                        ...
                        'lyrics_mu': np.array([o['zMean']]),
                        'lyrics_std': np.array([o['zStd']]),
                        'lyrics': o['line'],
                        'spec_id': o['audioClip']})
